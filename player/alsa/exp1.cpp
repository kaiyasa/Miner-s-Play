...
bool Init(const char *name)
{
	...
	if (name == NULL) {
		...
		err = snd_pcm_open(&_soundDevice, "default",
				 SND_PCM_STREAM_PLAYBACK, 0);
	} else {
		// Open the device we were told to open.
		err =
		    snd_pcm_open(&_soundDevice, name, SND_PCM_STREAM_PLAYBACK,
				 0);
	}
	...
	err =
	    snd_pcm_hw_params_set_rate_resample(_soundDevice, hw_params,
						resample);

	if (err < 0) {
		cout << "Init: Resampling setup failed for playback: " <<
		    snd_strerror(err) << endl;
	...
	if ((err =
	     snd_pcm_hw_params_set_access(_soundDevice, hw_params,
	...
	if ((err =
	     snd_pcm_hw_params_set_format(_soundDevice, hw_params,
	...
	if ((err =
	     snd_pcm_hw_params_set_channels(_soundDevice, hw_params, 2)) < 0) {
	...
	if ((err =
	     snd_pcm_hw_params_set_rate_near(_soundDevice, hw_params,
					     &actualRate, 0)) < 0) {

		cout << "Init: cannot set sample rate to 44100. (" <<
		    snd_strerror(err)
		    << ")" << endl;
	...
	if ((err = snd_pcm_hw_params(_soundDevice, hw_params)) < 0) {
		...
		cout << "Init: cannot set parameters (" << snd_strerror(err) <<
		    ")" << endl;
		...
		cout << "Audio device parameters have been set successfully." <<
		    endl;
